...
def check(x, p, q, r, s):
    if (('X' in x[1]) and ('X' in x[2]) and ('X' in x[3])) or (
        ('X' in x[4]) and ('X' in x[5]) and ('X' in x[6])) or (
            ('X' in x[7]) and ('X' in x[8]) and ('X' in x[9])) or (
                ('X' in x[1]) and ('X' in x[4]) and ('X' in x[7])) or (
                    ('X' in x[2]) and ('X' in x[5]) and ('X' in x[8])) or (
                        ('X' in x[3]) and ('X' in x[6]) and
                        ('X' in x[9])) or (('X' in x[1]) and ('X' in x[5]) and
                                           ('X' in x[9])) or (('X' in x[3]) and
                                                              ('X' in x[5]) and
                                                              ('X' in x[7])):
        print(f'congratulations!!!, {p} {s} is winner')
        return True

    elif (('O' in x[1]) and ('O' in x[2]) and ('O' in x[3])) or (
        ('O' in x[4]) and ('O' in x[5]) and ('O' in x[6])) or (
            ('O' in x[7]) and ('O' in x[8]) and ('O' in x[9])) or (
                ('O' in x[1]) and ('O' in x[4]) and
                ('O' in x[7])) or (('O' in x[2]) and ('O' in x[5]) and
                                   ('O' in x[8])) or (
                                       ('O' in x[3]) and ('O' in x[6]) and
                                       ('O' in x[9])) or (
                                           ('O' in x[1]) and ('O' in x[5]) and
                                           ('O' in x[9])) or (('O' in x[3]) and
                                                              ('O' in x[5]) and
                                                              ('O' in x[7])):
...
def play3(a, p, q, r, s, f):
    ...
    c = int(
        input(f'{p}{s}, enter the number where you want to place your mark:'))
    if c not in f:
        a[c] = "X"
    else:
...
def play4(a, p, q, r, s, f):
    ...
    c = int(
        input(f'{q}{r}, enter the number where you want to place your mark:'))
    if c not in f:
        a[c] = "O"
    else:
